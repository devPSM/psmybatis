...
public class MainController {
	 ...
	 * Dynamic Web Project : model2(MVC)를 구현시 httpServlet 상속클래스에서 HttpRequest 처리 --> web.xml
	 * spring에서 mapping 작업 없이 Annotation 만으로 servlet 구현
	 * 
	 * Portable (코드를 건들지 않고 다른 기술로 바꿀 수 있음)
	 * ex) 톰캣, 제티, 네티, 언더토우와 같은 아예 다른 기술 Stack을 소스코드 변경이 거의 없이 개발 가능
	 * 
	 * @Controller 안의 Mapping Annotaion (@getMapping, @PostMapping etc)
	 * Mapping Annotaion은 name, value, path, params, headers 등의 속성
	 * 1. 요청에 대한 정보를 공유 -> Handler Mapping 을 통해 해당 컨트롤러 선택
	 * 2. 이에 맵핑되는 요청일 때만 해당 메서드가 처리 : Handler Adapter
	 * 3. 해당 메서드는 view name으로 반환
